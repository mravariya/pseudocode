...
**Agent session log (local only):** at repo root, **`AGENT.local.md`** is **gitignored** (never pushed). Format and rules: **[docs/agent-session-log.md](docs/agent-session-log.md)** · Cursor rule **`.cursor/rules/agent-session-log.mdc`**. **Recent work (summary):** **[docs/recent-sessions.md](docs/recent-sessions.md)**.
